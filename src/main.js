...
import mediaAbi from "../contract/decentral.abi.json";
...
async function renderProducts() {
  ...
  const subs = await contract.methods
    .getSubscriptions(kit.defaultAccount)
    .call();
...
async function renderProfile() {
  const subs = await contract.methods
    .getSubscriptions(kit.defaultAccount)
    .call();
  const myProducts = await contract.methods.getMyPosts().call();
  document.getElementById("profile").innerHTML = "";
  const newDiv = document.createElement("div");
  newDiv.innerHTML = profileTemplate(kit.defaultAccount, subs, myProducts);
    ...
    <div class="card" style="width: 100%; border: solid 1px #000; margin-bottom: 20px;">
      <div class="card-body text-left position-relative">
        ...
        <div class="d-grid gap-2">
          <a class="btn btn-lg btn-outline-dark buyBtn fs-6 p-3" value="${
            _post.owner
          }">
...
function profileTemplate(_address, subs, myProducts) {
  return `
    <div class="card" style="width: 12rem; text-align: center; margin: auto;">
      ...
      <ul style="margin-top: -30px;" class="list-group list-group-flush">
        <li class="list-group-item">
          <span id="myInput">${_address.substring(0, 5)}...${_address.substring(
    _address.length - 3,
    _address.length
  )}</span>
        ...
        <li class="list-group-item center"><h4>Subscriptions</h4><span>${
          subs.length
        }</span></li>
        <li class="list-group-item center"><h4>My Tutorials</h4><span>${
          myProducts.length
        }</span></li>
      </ul>
    </div>
